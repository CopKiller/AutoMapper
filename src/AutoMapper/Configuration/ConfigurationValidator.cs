...
public readonly record struct ConfigurationValidator(IGlobalConfigurationExpression Expression)
{
    ...
    private void DryRunTypeMap(IGlobalConfiguration config, HashSet<TypeMap> typeMapsChecked, TypePair types,
        TypeMap typeMap, MemberMap memberMap)
    {
        if (typeMap == null)
        {
        ...
        if (typeMap != null)
        {
            ...
            Validate(new ValidationContext(types, memberMap, typeMap));
        ...
        else
        {
            ...
            Validate(new ValidationContext(types, memberMap, ObjectMapper: mapperToUse));
    ...
    private void CheckPropertyMaps(IGlobalConfiguration config, HashSet<TypeMap> typeMapsChecked, TypeMap typeMap)
    {
        foreach (var memberMap in typeMap.MemberMaps)
        {
            ...
            DryRunTypeMap(config, typeMapsChecked, new TypePair(sourceType, memberMap.DestinationType), null,
                memberMap);
        }
    }
}
