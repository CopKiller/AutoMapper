...
public abstract class TypeMapConfiguration(MemberList memberList, TypePair types)
{
    ...
    public Features<IMappingFeature> Features => _features ??= new Features<IMappingFeature>();
    ...
    protected void ReverseMapCore(TypeMapConfiguration reverseMap)
    {
        ...
        if (_memberConfigurations != null)
        {
            reverseMap.MemberConfigurations.AddRange(_memberConfigurations.Select(m => m.Reverse())
                .Where(m => m != null));
        }
    ...
    private void MapDestinationCtorToSource(TypeMap typeMap, List<MemberInfo> sourceMembers)
    {
        ...
        foreach (var destCtor in typeMap.DestinationConstructors)
        {
            ...
            foreach (var parameter in destCtor.Parameters)
            {
                ...
                sourceMembers.Clear();
                var canResolve = typeMap.Profile.MapDestinationPropertyToSource(typeMap.SourceTypeDetails,
                    constructor.DeclaringType, parameter.ParameterType, name, sourceMembers, IsReverseMap);
        ...
        {
            return _ctorParamConfigurations?.Any(c => c.CtorParamName == parameter.Name) is true;
        }
    }

    protected IEnumerable<IPropertyMapConfiguration> MapToSourceMembers()
    {
        return _memberConfigurations?.Where(m =>
            m.SourceExpression != null && m.SourceExpression.Body == m.SourceExpression.Parameters[0]) ?? [];
    }
    ...
    private void ReverseIncludedMembers(TypeMap typeMap)
    {
        ...
        foreach (var includedMember in typeMap.IncludedMembers.Where(i => i.IsMemberPath(out chain)))
        {
            ...
            ReverseSourceMembers(new MemberPath(chain), customExpression);
        }
    }
    ...
    private void ReverseSourceMembers(TypeMap typeMap)
    {
        foreach (var propertyMap in typeMap.PropertyMaps)
        {
            ...
            ReverseSourceMembers(new MemberPath(sourceMembers), customExpression);
        }
    }
    ...
    protected void ForSourceMemberCore(string sourceMemberName,
    ...
    protected void ForSourceMemberCore(MemberInfo memberInfo,
        Action<ISourceMemberConfigurationExpression> memberOptions)
    {
...
public abstract class
    MappingExpressionBase<TSource, TDestination, TMappingExpression>(MemberList memberList, TypePair types)
    : TypeMapConfiguration(memberList, types), IMappingExpressionBase<TSource, TDestination, TMappingExpression>
    where TMappingExpression : class, IMappingExpressionBase<TSource, TDestination, TMappingExpression>
{
    protected MappingExpressionBase(MemberList memberList) : this(memberList,
    ...
    {
        return BeforeMapCore((src, dest, ctxt) => beforeFunction(src, dest));
    }
    ...
    {
        return BeforeMapCore((src, dest, ctxt) => beforeFunction(src, dest, ctxt));
    }

    public TMappingExpression BeforeMap<TMappingAction>() where TMappingAction : IMappingAction<TSource, TDestination>
    {
        return BeforeMap(CallMapAction<TMappingAction>);
    }

    public TMappingExpression AfterMap<TMappingAction>() where TMappingAction : IMappingAction<TSource, TDestination>
    {
        return AfterMap(CallMapAction<TMappingAction>);
    }

    private static void CallMapAction<TMappingAction>(TSource source, TDestination destination,
        ResolutionContext context)
    {
        ((IMappingAction<TSource, TDestination>)context.CreateInstance(typeof(TMappingAction))).Process(source,
            destination, context);
    }

    public TMappingExpression AfterMap(Action<TSource, TDestination> afterFunction)
    {
        return AfterMapCore((src, dest, ctxt) => afterFunction(src, dest));
    }
    ...
    {
        return AfterMapCore((src, dest, ctxt) => afterFunction(src, dest, ctxt));
    }
    ...
    {
        TypeMapActions.Add(tm => { tm.DisableConstructorValidation = true; });
        return this as TMappingExpression;
    }
    ...
    public TMappingExpression ForSourceMember(string sourceMemberName,
        Action<ISourceMemberConfigurationExpression> memberOptions)
    {
    ...
    {
        return ConstructUsingCore(ctor);
    }
    ...
    public void ConvertUsing(Type typeConverterType)
    {
        ...
        TypeMapActions.Add(
            tm => tm.TypeConverter = new ClassTypeConverter(typeConverterType, tm.Types.ITypeConverter()));
    }
    ...
    public TMappingExpression ForCtorParam(string ctorParamName,
    ...
    {
        return type.GetRuntimeProperties().Where(p => p.GetSetMethod() == null);
    }
