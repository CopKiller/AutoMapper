...
{
    public MappingExpression(TypeMap typeMap) : this(typeMap.ConfiguredMemberList, typeMap.Types)
    {
        Projection = typeMap.Projection;
    }
    ...
    internal MemberConfigurationExpression ForMember(MemberInfo destinationProperty,
...
{
    public MappingExpression(MemberList memberList, bool projection) : base(memberList)
    {
        ...
        Action<IPathConfigurationExpression<TSource, TDestination, TMember>> memberOptions)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(destinationMember),
        ...
        {
            IgnoreDestinationMember(firstMember, false);
        }
    ...
    {
        MappingExpression<TDestination, TSource> reverseMap = new(MemberList.None, Types.Reverse())
            { IsReverseMap = true };
        ReverseMapCore(reverseMap);
    ...
    IProjectionExpression<TSource, TDestination> IProjectionExpression<TSource, TDestination>.ForMember<TMember>(
        Expression<Func<TDestination, TMember>> destinationMember,
        Action<IProjectionMemberConfiguration<TSource, TDestination, TMember>> memberOptions)
    {
        return (IProjectionExpression<TSource, TDestination>)ForMember(destinationMember, memberOptions);
    }

    IProjectionExpression<TSource, TDestination>
        IProjectionExpression<TSource, TDestination, IProjectionExpression<TSource, TDestination>>.AddTransform<TValue>(
            Expression<Func<TValue, TValue>> transformer)
    {
        return (IProjectionExpression<TSource, TDestination>)AddTransform(transformer);
    }

    IProjectionExpression<TSource, TDestination>
        IProjectionExpression<TSource, TDestination, IProjectionExpression<TSource, TDestination>>.IncludeMembers(
            params Expression<Func<TSource, object>>[] memberExpressions)
    {
        return (IProjectionExpression<TSource, TDestination>)IncludeMembers(memberExpressions);
    }

    IProjectionExpression<TSource, TDestination>
        IProjectionExpressionBase<TSource, TDestination, IProjectionExpression<TSource, TDestination>>.
        MaxDepth(int depth)
    {
        return (IProjectionExpression<TSource, TDestination>)MaxDepth(depth);
    }
    ...
    {
        return (IProjectionExpression<TSource, TDestination>)ValidateMemberList(memberList);
    }

    IProjectionExpression<TSource, TDestination>
        IProjectionExpressionBase<TSource, TDestination, IProjectionExpression<TSource, TDestination>>.ConstructUsing(
            Expression<Func<TSource, TDestination>> ctor)
    {
        return (IProjectionExpression<TSource, TDestination>)ConstructUsing(ctor);
    }

    IProjectionExpression<TSource, TDestination>
        IProjectionExpressionBase<TSource, TDestination, IProjectionExpression<TSource, TDestination>>.ForCtorParam(
            string ctorParamName, Action<ICtorParamConfigurationExpression<TSource>> paramOptions)
    {
        return (IProjectionExpression<TSource, TDestination>)ForCtorParam(ctorParamName, paramOptions);
    }
}