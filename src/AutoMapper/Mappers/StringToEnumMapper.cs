...
public sealed class StringToEnumMapper : IObjectMapper
{
    private static readonly MethodInfo EqualsMethod =
        typeof(StringToEnumMapper).GetMethod(nameof(StringCompareOrdinalIgnoreCase));

    private static readonly MethodInfo ParseMethod = typeof(Enum).StaticGenericMethod("Parse", 2);
    private static readonly PropertyInfo Length = typeof(string).GetProperty("Length");

    public bool IsMatch(TypePair context)
    {
        return context.SourceType == typeof(string) && context.DestinationType.IsEnum;
    }
        ...
        MemberMap memberMap, Expression sourceExpression, Expression destExpression)
    {
        ...
        return Condition(Equal(Property(sourceExpression, Length), Zero), configuration.Default(destinationType),
            enumMember);
    }

    internal static Expression CheckEnumMember(Expression sourceExpression, Type enumType, Expression defaultExpression,
        MethodInfo comparison = null)
    {
        ...
        foreach (var memberInfo in enumType.GetFields(TypeExtensions.StaticFlags))
        {
            ...
            var (body, testValue) = comparison == null
