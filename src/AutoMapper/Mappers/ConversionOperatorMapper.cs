...
public sealed class ConversionOperatorMapper : IObjectMapper
{
    ...
    {
        return GetConversionOperator(context.SourceType, context.DestinationType) != null;
    }

    private MethodInfo GetConversionOperator(Type sourceType, Type destinationType)
    {
        foreach (MethodInfo sourceMethod in sourceType.GetMember(_operatorName, MemberTypes.Method,
