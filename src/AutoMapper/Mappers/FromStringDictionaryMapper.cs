...
public sealed class FromStringDictionaryMapper : IObjectMapper
{
    ...
    {
        return Call(MapDynamicMethod, sourceExpression, destExpression.ToObject(), Constant(destExpression.Type),
            ContextParameter, Constant(profileMap));
    }

    private static object MapDynamic(StringDictionary source, object boxedDestination, Type destinationType,
        ...
        foreach (var member in profileMap.CreateTypeDetails(destinationType).WriteAccessors)
        {
            ...
            if (count > 1)
            {
                throw new AutoMapperMappingException(
                    ...
                    null, new TypePair(typeof(StringDictionary), destinationType));
            }
