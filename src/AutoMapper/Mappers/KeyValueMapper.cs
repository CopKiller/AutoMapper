...
public sealed class KeyValueMapper : IObjectMapper
{
    ...
    {
        return IsKeyValue(context.SourceType) && IsKeyValue(context.DestinationType);
    }

    public static bool IsKeyValue(Type type)
    {
        return type.IsGenericType(typeof(KeyValuePair<,>));
    }
        ...
        Expression sourceExpression, Expression destExpression)
    {
        ...
        var mapKeys =
            configuration.MapExpression(profileMap, keys, ExpressionBuilder.Property(sourceExpression, "Key"));
        var mapValues =
            configuration.MapExpression(profileMap, values, ExpressionBuilder.Property(sourceExpression, "Value"));
