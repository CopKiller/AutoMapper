...
public sealed class ConstructorMap
{
    ...
    {
        _ctorParams.Add(new ConstructorParameterMap(typeMap, parameter, sourceMembers.ToArray()));
    }
    ...
    public bool ApplyMap(TypeMap typeMap, IncludedMember includedMember = null)
    {
        ...
        foreach (var parameterMap in _ctorParams)
        {
            ...
            if (inheritedParameterMap is not { IsMapped: true, DestinationType: var type } ||
                type != parameterMap.DestinationType || !parameterMap.ApplyMap(inheritedParameterMap, includedMember))
            {
                continue;
            }
...
public class ConstructorParameterMap : MemberMap
{
    ...
    {
        return Parameter.IsOptional ? Parameter.GetDefaultValue(configuration) : configuration.Default(DestinationType);
    }

    public override string ToString()
    {
        return $"{Parameter.Member}, parameter {DestinationName}";
    }
