...
{
    private static readonly MethodInfo SelectMethod = typeof(Enumerable).StaticGenericMethod("Select", 2);
        ...
        Expression resolvedSource, LetPropertyMaps letPropertyMaps)
    {
        ...
        if (sourceListType != destinationListType)
        {
            ...
            var transformedExpressions =
                configuration.ProjectionBuilder.CreateProjection(itemRequest, letPropertyMaps.New());
    ...
    {
        return Call(SelectMethod.MakeGenericMethod(lambda.Parameters[0].Type, lambda.ReturnType), source, lambda);
    }
}