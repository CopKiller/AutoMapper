...
public sealed class TypeDetails(Type type, ProfileMap config)
{
    ...
    public MemberInfo GetMember(string name)
    {
        ...
        if (_nameToMember.TryGetValue(name, out var member) && Config.MethodMappingEnabled &&
        ...
        {
            _nameToMember = new Dictionary<string, MemberInfo>(ReadAccessors.Length, StringComparer.OrdinalIgnoreCase);
        ...
        {
            return Type.GetMethods(BindingFlags.Instance | BindingFlags.Public).Where(m =>
                m.DeclaringType != typeof(object) && m.ReturnType != typeof(void) && !m.IsGenericMethodDefinition &&
        ...
        IEnumerable<MemberInfo> GetNoArgExtensionMethods(IEnumerable<MethodInfo> sourceExtensionMethodSearch)
        {
            var extensionMethods = (IEnumerable<MemberInfo>)
                sourceExtensionMethodSearch.Where(method =>
                    !method.ContainsGenericParameters && method.FirstParameterType().IsAssignableFrom(Type));
    ...
    private sealed class GenericMethod(MethodInfo genericMethod, Type genericInterface) : MemberInfo
    {
        private readonly MethodInfo _genericMethod = genericMethod;
        private readonly Type _genericInterface = genericInterface;
        private MethodInfo _closedMethod = ObjectToString;
    ...
    {
        return Type.IsInterface ? Type.GetInterfaces().Prepend(Type) : Type.GetTypeInheritance();
    }

    private IEnumerable<PropertyInfo> GetProperties(Func<PropertyInfo, bool> propertyAvailableFor)
    {
        return GetTypeInheritance().SelectMany(type =>
            type.GetProperties(TypeExtensions.InstanceFlags).Where(property =>
