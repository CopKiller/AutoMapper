...
public class MemberMap : IValueResolver
{
    ...
    protected MemberMap(TypeMap typeMap, Type destinationType)
    {
        (TypeMap, DestinationType) = (typeMap, destinationType);
    }
    ...
    {
        SetResolver(new ExpressionResolver(lambda));
    }
    ...
    public virtual IncludedMember IncludedMember
    {
        get => default;
        protected set { }
    }
    ...
    {
        return Resolver?.SourceMemberName ?? SourceMember?.Name;
    }
    ...
    {
        return SourceMembers.Chain(source);
    }
    ...
    {
        return Resolver?.ResolvedType ?? DestinationType;
    }
    ...
    Expression IValueResolver.GetExpression(IGlobalConfiguration configuration, MemberMap memberMap, Expression source,
        Expression destination, Expression destinationMember)
    {
        return ChainSourceMembers(configuration, source, destinationMember);
    }

    MemberInfo IValueResolver.GetSourceMember(MemberMap memberMap)
    {
        return SourceMembers[0];
    }
...
public readonly record struct ValueTransformerConfiguration(Type ValueType, LambdaExpression TransformerExpression)
{
    ...
    {
        return ValueType.IsAssignableFrom(memberMap.SourceType) &&
...
public static class ValueTransformerConfigurationExtensions
{
    ...
    public static void Add<TValue>(this List<ValueTransformerConfiguration> valueTransformers,
