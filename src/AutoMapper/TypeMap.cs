...
{
    private static readonly LambdaExpression EmptyLambda = Lambda(ExpressionBuilder.Empty);
    ...
    public TypeMap(Type sourceType, Type destinationType, ProfileMap profile, TypeMapConfiguration typeMapConfiguration,
        List<MemberInfo> sourceMembers = null)
    {
        ...
        foreach (var destinationProperty in DestinationTypeDetails.WriteAccessors)
        {
            ...
            if (profile.MapDestinationPropertyToSource(SourceTypeDetails, destinationType, propertyType,
                    destinationName, sourceMembers, isReverseMap))
            {
    ...
    {
        return ConstructorMap?.Ctor is ConstructorInfo ctor && ctor.IsFamily && ctor.Has<CompilerGeneratedAttribute>()
    ...
    private TypeMapDetails Details => _details ??= new TypeMapDetails();
    ...
    public void CheckProjection()
    {
        if (Projection)
        {
            throw new AutoMapperConfigurationException("CreateProjection works with ProjectTo, not with Map.",
    ...
    {
        return MissingMapException(types.SourceType, types.DestinationType);
    }
    ...
    {
        return Expression.Invoke(MapExpression, ToType(source, SourceType), ToType(destination, DestinationType),
            ContextParameter);
    }

    internal bool CanConstructorMap()
    {
        return Profile.ConstructorMappingEnabled && !DestinationType.IsAbstract &&
    ...
    public LambdaExpression CustomCtorExpression =>
        CustomCtorFunction is { Parameters: [_] } ? CustomCtorFunction : null;
    ...
    public Type MakeGenericType(Type type)
    {
        return type.IsGenericTypeDefinition
            ? type.MakeGenericType(SourceType.GenericTypeArguments.Concat(DestinationType.GenericTypeArguments)
                .Take(type.GenericParametersCount()).ToArray())
            : type;
    }
    ...
    {
        return IncludedMembersNames.Length == 0 || SourceType.ContainsGenericParameters
            ? IncludedMembers
            : IncludedMembers.Concat(IncludedMembersNames.Select(name => MemberAccessLambda(SourceType, name, this)));
    }

    public bool ConstructorParameterMatches(string destinationPropertyName)
    {
        return ConstructorMapping && ConstructorMap[destinationPropertyName] != null;
    }
    ...
    public string[] GetUnmappedPropertyNames()
    {
        ...
        IEnumerable<MemberMap> MappedMembers()
        {
            return MemberMaps.Where(pm => pm.IsMapped);
        }
    }
    ...
    public PropertyMap FindOrCreatePropertyMapFor(MemberInfo destinationProperty, Type destinationPropertyType)
    {
        ...
        if (propertyMap == null)
        {
            propertyMap = new PropertyMap(destinationProperty, destinationPropertyType, this);
            AddPropertyMap(propertyMap);
        }
    ...
    {
        Details.AddAfterMapAction(afterMap);
    }
    ...
    {
        return GetPropertyMap(propertyMap.DestinationName);
    }
    ...
    {
        return Details.AddMemberMap(includedMember);
    }

    public PathMap FindOrCreatePathMapFor(LambdaExpression destinationExpression, MemberPath path, TypeMap typeMap)
    {
        return Details.FindOrCreatePathMapFor(destinationExpression, path, typeMap);
    }
    ...
    {
        return Details.FindOrCreateSourceMemberConfigFor(sourceMember);
    }

    private class TypeMapDetails
    {
        ...
        public Features<IRuntimeFeature> Features => _features ??= new Features<IRuntimeFeature>();
        ...
        private void AddPathMap(PathMap pathMap)
        {
        ...
        private void ApplyIncludedMemberTypeMap(IncludedMember includedMember, TypeMap thisMap)
        {
            ...
            if (details != null)
            {
                ApplyInheritedMapActions(details.BeforeMapActions?.Select(includedMember.Chain),
                    details.AfterMapActions?.Select(includedMember.Chain));
            }

            foreach (var notOverridenPathMap in notOverridenPathMaps)
            {
                AddPathMap(new PathMap(notOverridenPathMap, thisMap, includedMember));
            }
        }
        ...
        private void ApplyInheritedTypeMap(TypeMap inheritedTypeMap, TypeMap thisMap)
        {
            ...
            void ApplyInheritedPropertyMaps(TypeMap inheritedTypeMap, TypeMap thisMap)
            {
                foreach (var inheritedMappedProperty in inheritedTypeMap._propertyMaps)
                {
                    ...
                    {
                        thisMap.AddPropertyMap(new PropertyMap(inheritedMappedProperty, thisMap));
                    }
                }
            }
        ...
        private void ApplyInheritedMapActions(IEnumerable<LambdaExpression> beforeMap,
