...
public static class ExpressionBuilder
{
    ...
    private static readonly MethodInfo ContextCreate =
        typeof(ResolutionContext).GetInstanceMethod(nameof(ResolutionContext.CreateInstance));

    private static readonly MethodInfo OverTypeDepthMethod =
    ...
    private static readonly Expression DisposeCall =
        IfThen(ReferenceNotEqual(Disposable, Null), Expression.Call(Disposable, DisposeMethod));
    ...
    public static Expression ReplaceParameters(this IGlobalConfiguration configuration, LambdaExpression initialLambda,
        Expression newParameter)
    {
        return configuration.ParameterReplaceVisitor().Replace(initialLambda, newParameter);
    }

    public static Expression ReplaceParameters(this IGlobalConfiguration configuration, LambdaExpression initialLambda,
        Expression[] newParameters)
    {
        return configuration.ParameterReplaceVisitor().Replace(initialLambda, newParameters);
    }

    public static Expression ConvertReplaceParameters(this IGlobalConfiguration configuration,
        LambdaExpression initialLambda, Expression newParameter)
    {
        return configuration.ConvertParameterReplaceVisitor().Replace(initialLambda, newParameter);
    }

    public static Expression ConvertReplaceParameters(this IGlobalConfiguration configuration,
        LambdaExpression initialLambda, Expression[] newParameters)
    {
        return configuration.ConvertParameterReplaceVisitor().Replace(initialLambda, newParameters);
    }

    public static DefaultExpression Default(this IGlobalConfiguration configuration, Type type)
    {
        return configuration == null ? Expression.Default(type) : configuration.GetDefault(type);
    }
    ...
    public static Expression MapExpression(this IGlobalConfiguration configuration, ProfileMap profileMap,
        TypePair typePair, Expression source,
        MemberMap memberMap = null, Expression destination = null)
    {
        ...
        return nullCheck
            ? configuration.NullCheckSource(profileMap, source, destination, mapExpression, memberMap)
            : mapExpression;
    }

    public static Expression NullCheckSource(this IGlobalConfiguration configuration, ProfileMap profileMap,
        Expression source, Expression destination,
        Expression mapExpression, MemberMap memberMap)
    {
        ...
        {
            if ((isCollection && profileMap.AllowsNullCollectionsFor(memberMap)) ||
    ...
    {
        return Expression.Call(ContextParameter, ContextCreate, Constant(type));
    }

    public static Expression ContextMap(TypePair typePair, Expression sourceParameter, Expression destinationParameter,
        MemberMap memberMap)
    {
    ...
    {
        return typeMap.PreserveReferences || typeMap.MaxDepth > 0 ? CheckContextCall : null;
    }

    public static Expression OverMaxDepth(TypeMap typeMap)
    {
        return typeMap?.MaxDepth > 0 ? Expression.Call(ContextParameter, OverTypeDepthMethod, Constant(typeMap)) : null;
    }

    public static Expression NullSubstitute(this MemberMap memberMap, Expression sourceExpression)
    {
        return Coalesce(sourceExpression, ToType(Constant(memberMap.NullSubstitute), sourceExpression.Type));
    }
        ...
        IGlobalConfiguration configuration)
    {
        ...
        return perMember.Count > 0 || perMap.Count > 0 || perProfile.Count > 0
            ? memberMap.ApplyTransformers(source, configuration, perMember.Concat(perMap).Concat(perProfile))
            : source;
    }

    private static Expression ApplyTransformers(this MemberMap memberMap, Expression result,
        IGlobalConfiguration configuration, IEnumerable<ValueTransformerConfiguration> transformers)
    {
        foreach (var transformer in transformers)
        {
            ...
            {
                result = ToType(
                    configuration.ReplaceParameters(transformer.TransformerExpression,
                        ToType(result, transformer.ValueType)), memberMap.DestinationType);
            }
        }
    ...
    public static LambdaExpression Lambda(this MemberInfo member)
    {
        return new[] { member }.Lambda();
    }
    ...
    {
        return lambda.Body.GetChain().ToMemberInfos();
    }
    ...
    public static Stack<Member> GetChain(this Expression expression)
    {
        ...
        while (expression != null)
        {
            var member = expression switch
            {
                ...
                MethodCallExpression { Method: var extensionMethod, Arguments: { Count: > 0 } arguments } when
                    extensionMethod.Has<ExtensionAttribute>() =>
    ...
    {
        return GetMemberPath(type, memberPath, typeMap).Lambda();
    }

    public static Expression ForEach(List<ParameterExpression> variables, List<Expression> statements,
        ParameterExpression loopVar, Expression collection, Expression loopContent)
    {
        ...
        static Expression ForEachArrayItem(List<ParameterExpression> variables, List<Expression> statements,
            ParameterExpression loopVar, Expression array, Expression loopContent)
        {
            var breakLabel = Label("LoopBreak");
            variables.Add(Index);
            variables.Add(loopVar);
    ...
    {
        return Expression.Property(target, target.Type.GetInheritedProperty(name));
    }
    ...
    {
        return Expression.Call(target, target.Type.GetInheritedMethod(name));
    }

    public static MethodCallExpression Call(Expression target, string name, Expression[] arguments)
    {
        return Expression.Call(target, target.Type.GetInheritedMethod(name), arguments);
    }
    ...
    {
        return new ParameterReplaceVisitor().Replace(initialLambda, newParameter);
    }

    private static Expression Replace(this ParameterReplaceVisitor visitor, LambdaExpression initialLambda,
        Expression newParameter)
    {
        return visitor.Replace(initialLambda.Body, initialLambda.Parameters[0], newParameter);
    }
    ...
    {
        return new ReplaceVisitor().Replace(exp, old, replace);
    }

    public static Expression NullCheck(this Expression expression, IGlobalConfiguration configuration,
        MemberMap memberMap = null, Expression defaultValue = null, IncludedMember includedMember = null)
    {
        ...
        var returnType =
            destinationType != null && destinationType != expression.Type &&
            Nullable.GetUnderlyingType(destinationType) == expression.Type
                ? destinationType
                : expression.Type;
        var defaultReturn = defaultValue is { NodeType: ExpressionType.Default } && defaultValue.Type == returnType
            ? defaultValue
        ...
        static MethodCallExpression ExtensionMethod(MethodInfo method, Expression newTarget,
...
public class ReplaceVisitor : ReplaceVisitorBase
{
    ...
    {
        return _oldNode == node ? _newNode : base.Visit(node);
    }
}
...
public class ParameterReplaceVisitor : ReplaceVisitorBase
{
    ...
    {
        return _oldNode == node ? _newNode : base.VisitParameter(node);
    }
}
...
public class ConvertParameterReplaceVisitor : ParameterReplaceVisitor
{
    ...
    {
        return base.Replace(target, oldNode, ToType(newNode, oldNode.Type));
    }
}