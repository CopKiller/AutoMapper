...
{
    private static readonly Expression EmptyString = Constant(string.Empty);
    ...
    {
        return Lambda<Func<object>>(GenerateConstructorExpression(type, null).ToObject()).Compile();
    }
    ...
    private static Expression CallConstructor(Type type, IGlobalConfiguration configuration)
    {
        ...
        var ctorWithOptionalArgs =
            (from ctor in type.GetDeclaredConstructors()
                ...
                select (ctor, args)).FirstOrDefault();
    ...
    {
        return type.GetIEnumerableType()?.GenericTypeArguments ?? [typeof(object)];
    }
