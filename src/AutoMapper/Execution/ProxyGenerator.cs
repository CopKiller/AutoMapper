...
{
    private static readonly MethodInfo DelegateCombine =
        typeof(Delegate).GetMethod(nameof(Delegate.Combine), [typeof(Delegate), typeof(Delegate)]);
    ...
    private static Type EmitProxy(TypeDescription typeDescription)
    {
        ...
        {
            propertyChangedField = typeBuilder.DefineField(PropertyChanged.Name, typeof(PropertyChangedEventHandler),
        ...
        void GenerateFields()
        {
            ...
            foreach (var property in PropertiesToImplement())
            {
                if (fieldBuilders.TryGetValue(property.Name, out var propertyEmitter))
                {
                    ...
                    {
                        throw new ArgumentException($"The interface has a conflicting property {property.Name}",
    ...
    {
        return ProxyTypes.GetOrAdd(new TypeDescription(interfaceType, []));
    }

    public static Type GetSimilarType(Type sourceType, IEnumerable<PropertyDescription> additionalProperties)
    {
        return ProxyTypes.GetOrAdd(new TypeDescription(sourceType, [..additionalProperties.OrderBy(p => p.Name)]));
    }
