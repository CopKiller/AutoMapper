...
public ref struct TypeMapPlanBuilder(IGlobalConfiguration configuration, TypeMap typeMap)
{
    ...
    private static readonly MethodInfo IncTypeDepthInfo =
        typeof(ResolutionContext).GetInstanceMethod(nameof(ResolutionContext.IncrementTypeDepth));

    private static readonly MethodInfo DecTypeDepthInfo =
        typeof(ResolutionContext).GetInstanceMethod(nameof(ResolutionContext.DecrementTypeDepth));

    private static readonly MethodInfo CacheDestinationMethod =
        typeof(ResolutionContext).GetInstanceMethod(nameof(ResolutionContext.CacheDestination));

    private static readonly MethodInfo GetDestinationMethod =
        typeof(ResolutionContext).GetInstanceMethod(nameof(ResolutionContext.GetDestination));

    private readonly IGlobalConfiguration _configuration = configuration;
    private readonly ParameterExpression _destination = Variable(typeMap.DestinationType, "typeMapDestination");
    private readonly ParameterExpression _initialDestination = Parameter(typeMap.DestinationType, "destination");
    private readonly ParameterExpression[] _parameters = configuration.Parameters ?? [null, null, ContextParameter];
    private readonly TypeMap _typeMap = typeMap;
    private readonly ParameterExpression _source = Parameter(typeMap.SourceType, "source");
    private List<ParameterExpression> _variables = configuration.Variables;
    private List<Expression> _expressions = configuration.Expressions;
    private CatchBlock[] _catches = configuration.Catches;
    ...
    {
        return new AutoMapperMappingException("Error mapping types.", innerException, memberMap);
    }

    private ParameterExpression[] GetParameters(ParameterExpression first = null, ParameterExpression second = null)
    {
    ...
    {
        return _configuration.ReplaceParameters(lambda, GetParameters());
    }
    ...
    private Expression DestinationMemberValue(MemberMap memberMap, Expression destinationMemberGetter,
        bool destinationMemberReadOnly)
    {
    ...
    {
        var preCondition = _configuration.ConvertReplaceParameters(memberMap.PreCondition, GetParameters(customSource));
        var ifThen = IfThen(preCondition, Block(_expressions));
        _expressions.Clear();
        _expressions.Add(ifThen);
    }

    private ParameterExpression SetVariables(Expression valueResolver, ParameterExpression resolvedValueVariable,
    ...
    private Expression MapMember(MemberMap memberMap, ParameterExpression resolvedValue,
    ...
    {
        return memberMap.IncludedMember?.Variable ?? _source;
    }
}
...
public class MemberPathResolver(MemberInfo[] members) : IValueResolver
{
    ...
    {
        return _members.Length == 1 ? _members[0] : null;
    }
    ...
    {
        return _members[0].DeclaringType.ContainsGenericParameters
...
public class FuncResolver(LambdaExpression lambda) : LambdaValueResolver(lambda), IValueResolver
{
    public Expression GetExpression(IGlobalConfiguration configuration, MemberMap memberMap, Expression source,
        Expression destination, Expression destinationMember)
    {
        return configuration.ConvertReplaceParameters(Lambda,
            [source, destination, destinationMember, ContextParameter]);
    }

    public MemberInfo GetSourceMember(MemberMap _)
    {
        return null;
    }
}
...
public class ValueConverter : ValueResolverConfig, IValueResolver
{
        ...
        Expression _, Expression destinationMember)
    {
        ...
        {
            { SourceMemberLambda: not null } => configuration.ReplaceParameters(SourceMemberLambda, source),
            { SourceMemberName: not null } => PropertyOrField(source, SourceMemberName),
        ...
        {
            return new AutoMapperConfigurationException(
...
public class ClassValueResolver : ValueResolverConfig, IValueResolver
{
                        ...
                        new[]
 {
    ...
    public MemberInfo GetSourceMember(MemberMap _)
    {
        return SourceMemberLambda?.GetMember();
    }
}
...
public class LambdaTypeConverter(LambdaExpression lambda) : TypeConverter
{
    ...
    {
        return configuration.ConvertReplaceParameters(Lambda, parameters);
    }
}
...
public class ClassTypeConverter(Type converterType, Type converterInterface) : TypeConverter
{
    ...
    public override Expression GetExpression(IGlobalConfiguration configuration, ParameterExpression[] parameters)
    {
        return Call(ToType(ServiceLocator(ConverterType), ConverterInterface), "Convert", parameters);
    }
